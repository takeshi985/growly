...
На первом запуске устройство выбирает роль. Детский режим содержит только обучение, карту уровней, подсказки и экран кода для родителя. Родительский режим содержит только подключение по коду, прогресс и карту обучения. Выбор и идентификаторы устройств сохраняются локально через `shared_preferences`.
